  ...
  return (
    <div className="h-screen flex flex-col m-0 p-0 justify-between">
      <header className="flex p-3 text-xl">
        <Button variant="ghost" size="icon"><PanelLeftOpen color="#555555" style={{ width: 20, height: 20 }} /></Button>
      ...
      <div className="w-full max-w-3xl m-auto">
        ...
        <div className="flex gap-2 justify-center">
          {
            optionButtons.map(({ text, icon, color }) => <OptionButton Icon={icon} color={color}>{text}</OptionButton>)
          }
        </div>
      </div>
      <p className="text-center text-xs mb-6 text-gray-700">ChatGPT can make mistakes. Check important info.</p>
    </div>
  );
}
